...
export const proofOfCommission = catchAsyncErrors(async (req, res, next) => {
  ...
  if (user.unpaidCommission < amount) {
    return next(
      new ErrorHandler(
        `The amount exceeds your unpaid comission balance.Please enter an amount up to ${user.unpaidCommission}`,
        403
      )
    );
  }
  ...
  if (!allowedFormats.includes(proofOfCommission.mimetype)) {
...
export const calculateCommission=async(auctionId)=>{
  ...
  //user.unpaidCommission=commission;
  return commission;
