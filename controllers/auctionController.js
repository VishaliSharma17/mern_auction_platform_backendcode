...
export const addNewAuctionItem = catchAsyncErrors(async (req, res, next) => {
  ...
  if (alreadyOneAuctionActive.length > 0) {
    return next(
      new ErrorHandler(
        "You have already have one active auction.",
        400
      )
    );
  }
  try {
    ...
    return res.status(201).json({
      success: true,
      message: `Auction item craeted and will be listed on auction page at ${startTime}`,
      auctionItem,
    });
  } catch (error) {
    return next(
...
export const republishItem = catchAsyncErrors(async (req, res, next) => {
  // Check if the user has unpaid commission
const user = await User.findById(req.user._id);
if (user.unpaidCommission > 0) {
  return next(
    new ErrorHandler(
      `You have unpaid commission of ₹${user.unpaidCommission}. Please clear it before republishing.`,
      403
    )
  );
}
  ...
  const createdBy=await User.findByIdAndUpdate(req.user._id,data,{
    unpaidCommission:0,
    new:true,
    runValidators:false,
    useFindAndModify:false,
